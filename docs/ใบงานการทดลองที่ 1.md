...
1. ศึกษาวงจรของ Sensor Node สำหรับสถานีฐานโทรคมนาคม ดังภาพ

   ![Sensor Node Block Diagram](images/lab-1/sensor-node-block.svg)

   สถานีฐาน Site A-01 ประกอบด้วยเซ็นเซอร์วัดสิ่งแวดล้อม 3 ชนิด:
...
4. เขียนโปรแกรม — เปลี่ยนจาก AP Mode เป็น STA Mode เพื่อเชื่อมต่อ WiFi Router จริง
   ...
   String checkStatus() {
     if (temp > 35 || hum > 80 || light < 1000) return "ALARM";
     if (temp > 32 || hum > 70 || light < 2000) return "WARNING";
     return "NORMAL";
   }
   ...
   void setup() {
     ...
     server.on("/", []() {
       ...
       html += "<style>"
               ...
               "</style></head><body>";
       html += "<h1>Site A-01</h1>";
       html += "<p>Status: <span class='" + statusStr.toLowerCase() + "'>" + statusStr + "</span></p>";
       html += "<p>Temp: " + String(temp) + " C</p>";
       html += "<p>Humidity: " + String(hum) + " %</p>";
       html += "<p>Light: " + String(light) + "</p>";
       html += "</body></html>";
...
1. การจำแนกระดับ Alarm มีประโยชน์ต่อการดำเนินงานของสถานีฐานโทรคมนาคมอย่างไร
...
2. เหตุใดต้องตรวจสอบทั้งอุณหภูมิ ความชื้น และแสงในสถานีฐาน
3. การใช้ ESP32 เป็น Web Server มีข้อดีและข้อเสียอย่างไรเมื่อเทียบกับการส่งข้อมูลไปยัง Server กลาง
4. ถ้าสถานีฐานอยู่ในพื้นที่กลางแจ้ง (Outdoor Cabinet) ควรเพิ่มเซ็นเซอร์ใดบ้าง
